...
public class SXSSFExcelUtil<T> {
    ...
    public SXSSFExcelUtil(Class <T> clazz) {
        this.clazz=clazz;
     ...
     * @param sheet
     */
    private void setHSSFCellStyle(List <Field> fields, SXSSFRow row, SXSSFCell cell, SXSSFSheet sheet){
        // 写入各个字段的列头名称
    ...
    public CellStyle getHeadCellStyle() {
        if (this.headCellStyle != null) {
            ...
            Font headFont = this.workbook.createFont();
            headFont.setBold(true);
            this.headCellStyle.setFont(headFont);
            this.headCellStyle.setBorderBottom(BorderStyle.valueOf((short)1));
            this.headCellStyle.setBorderLeft(BorderStyle.valueOf((short)1));
            this.headCellStyle.setBorderRight(BorderStyle.valueOf((short)1));
            this.headCellStyle.setBorderTop(BorderStyle.valueOf((short)1));
            return this.headCellStyle;
        }
    }
    ...
    public CellStyle getStringCellStyle() {
        if (this.stringCellStyle != null) {
            ...
            this.stringCellStyle.setBorderBottom(BorderStyle.valueOf((short)1));
            this.stringCellStyle.setBorderLeft(BorderStyle.valueOf((short)1));
            this.stringCellStyle.setBorderRight(BorderStyle.valueOf((short)1));
            this.stringCellStyle.setBorderTop(BorderStyle.valueOf((short)1));
            DataFormat format = this.workbook.createDataFormat();
            this.stringCellStyle.setDataFormat(format.getFormat(this.stringFormat));
            return this.stringCellStyle;
        }
    }
    /**
     ...
     * @param sheet
     */
    private void setData(int index,List<T> list,List<Field> fields,SXSSFRow row,SXSSFCell cell,SXSSFSheet sheet){
        ...
        for (int i=startNo; i < endNo; i++) {
            row=(SXSSFRow)sheet.createRow(i + 1 - startNo);
            T vo=(T) list.get(i);
                ...
                try {
                    ...
                    if (attr.isExport()) {
                        ...
                        try {
                            ...
                            cell.setCellType(CellType.NUMERIC.getCode());
     ...
     * @return 设置好的sheet.
     */
    public static SXSSFSheet setHSSFPrompt(SXSSFSheet sheet, String promptTitle, String promptContent, int firstRow, int endRow, int firstCol, int endCol) {
        // 构造constraint对象
        DataValidationHelper helper = sheet.getDataValidationHelper();
        DataValidationConstraint constraint = helper.createFormulaListConstraint("DD1");
        // 四个参数分别是：起始行、终止行、起始列、终止列
        CellRangeAddressList regions=new CellRangeAddressList(firstRow, endRow, firstCol, endCol);
        // 数据有效性对象
        DataValidation dataValidationView= helper.createValidation(constraint, regions);
        dataValidationView.createPromptBox(promptTitle, promptContent);
        // 处理Excel兼容性问题
        if (dataValidationView instanceof XSSFDataValidation) {
            // 数据校验
            dataValidationView.setSuppressDropDownArrow(true);
            dataValidationView.setShowErrorBox(true);
        } else {
            dataValidationView.setSuppressDropDownArrow(false);
        }
    ...
    public String encodingFilename(String filename) {
        filename=filename+"_"+DateUtil.format(new Date(),DatePattern.PURE_DATETIME_MS_PATTERN) + ".xlsx";
        return filename;
    }
