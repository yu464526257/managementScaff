...
import com.yjc.system.commen.common.exception.CheckedException;
import com.yjc.system.commen.entity.ColumnEntity;
import com.yjc.system.commen.entity.GenConfigEntity;
import com.yjc.system.commen.entity.TableEntity;
import com.yjc.system.commen.common.enums.CommonConstants;
import com.yjc.system.commen.common.exception.CheckedException;
...
public class GenUtils {
	 ...
	 * 生成代码
	 */
	public static void genCode(GenConfigEntity genConfig, Map<String, String> table,
		...
		//权限表名去前缀
		String pmsName = tableToJavaNoPrefix(tableEntity.getTableName(), tablePrefix);
		...
		for (Map<String, String> column : columns) {
            ...
            columnEntity.setDataScale(StringUtils.isNotNull(column.get("DATASCALE"))?Integer.valueOf(column.get("DATASCALE").toString()):0);
            columnEntity.setDataPrecision(StringUtils.isNotNull(column.get("DATAPRECISION"))?Integer.valueOf(column.get("DATAPRECISION").toString()):0);

			//列名转换成Java属性名
//			String attrName = columnToJava(columnEntity.getColumnName());
			String attrName = columnEntity.getColumnName();
			columnEntity.setCaseAttrName(attrName);
			attrName= StringUtils.swapCase(attrName);
			String[] nameList=attrName.split("_");
			String newColumnNam="";
			for(String i:nameList){
			    if(i.equals(nameList[0])){
                    newColumnNam=i;
			        continue;
                }
                newColumnNam=newColumnNam+StringUtils.capitalize(i);
            }

            columnEntity.setLowerAttrName(newColumnNam);
			String attrType="";
			if(columnEntity.getDataScale()>0 && "NUMBER".equals(columnEntity.getDataType())){
				columnEntity.setDataType("NUMBER(P,S)");
			}
			if(columnEntity.getDataScale()==0 && columnEntity.getDataPrecision()>6 && "NUMBER".equals(columnEntity.getDataType())){
				columnEntity.setDataType("NUMBER(P)");
			}
			//列的数据类型，转换成Java类型
	 ...
	 * 表名转换成Java类名
	 */
	private static String tableToJavaNoPrefix(String tableName, String tablePrefix) {
		if (StringUtils.isNotBlank(tablePrefix)) {
			tableName = tableName.replace(tablePrefix, "");
		}
		return tableName;
	...
	private static Configuration getConfig() {
		try {
//			return new PropertiesConfiguration("F:\\JiShuBao\\logisticsNew\\target\\classes\\gen.properties");
			return new PropertiesConfiguration("E:\\work\\svnWorkPace\\managementSaleStock\\system-admin\\target\\classes\\gen.properties");
